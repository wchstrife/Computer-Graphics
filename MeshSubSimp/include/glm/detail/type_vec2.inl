...
#	endif//!GLM_HAS_DEFAULTED_FUNCTIONS
    ...
    tvec2<T, P>::operator[](typename tvec2<T, P>::length_type i)
    {
    ...
    tvec2<T, P>::operator[](typename tvec2<T, P>::length_type i) const
    {
        assert(i >= 0 && i < this->length());
        return (&x)[i];
    }
...
#	if !GLM_HAS_DEFAULTED_FUNCTIONS
    ...
    &
    tvec2<T, P>::operator=(tvec2<T, P> const
    & v)
{
    this->
    x = v.x;
...
GLM_FUNC_QUALIFIER tvec2<T, P>
&
tvec2<T, P>::operator=(tvec2<U, P> const
& v)
{
this->
x = static_cast<T>(v.x);
...
operator+(T scalar, tvec2<T, P> const &v)
{
    return tvec2<T, P>(
            scalar + v.x,
            scalar + v.y);
...
operator+(tvec1 <T, P> const &v1, tvec2<T, P> const &v2)
{
    return tvec2<T, P>(
            v1.x + v2.x,
            v1.x + v2.y);
